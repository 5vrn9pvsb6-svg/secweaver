...
```text
Web 入口 → shell 子进程 → 后续命令
```

下载执行样例验证命令检测流程，不证明所有内存 WebShell 均可检出，也不直接证明载荷驻留内存。

调查时记录资产范围、带时区的时间窗、数据缺口和每项结论对应的证据引用。采集或查询不完整时保留“无法判断”，不能以未发现事件等同于未发生攻击。

## 公开离线练习

以下[合成样例](../../examples/risk-identification/s5-curl-download-exec-p0.json)用于练习当前分析接口，**不是历史实验的原始输入**。完成 [快速上手](../../docs_user/00-security-operator-quickstart.zh-CN.md) 的 Python 安装后，从仓库根目录执行：

```bash
python3 src/skills/risk-identification/scripts/assess.py \
  -i examples/risk-identification/s5-curl-download-exec-p0.json \
  -o /tmp/secweaver-case-2.json
```

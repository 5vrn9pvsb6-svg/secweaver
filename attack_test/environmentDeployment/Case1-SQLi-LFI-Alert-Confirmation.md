...
```text
WAF 告警 → Web 请求 → 主机执行证据
```

SQLi 被阻断不能推出主机已失陷；应区分告警真实性与攻击是否成功。

调查时记录资产范围、带时区的时间窗、数据缺口和每项结论对应的证据引用。采集或查询不完整时保留“无法判断”，不能以未发现事件等同于未发生攻击。

## 公开离线练习

以下[合成样例](../../examples/alert-confirmation/s4-sqli-blocked-no-breach.json)用于练习当前分析接口，**不是历史实验的原始输入**。完成 [快速上手](../../docs_user/00-security-operator-quickstart.zh-CN.md) 的 Python 安装后，从仓库根目录执行：

```bash
python3 src/skills/alert-confirmation/scripts/confirm.py \
  -i examples/alert-confirmation/s4-sqli-blocked-no-breach.json \
  -o /tmp/secweaver-case-1.json
```

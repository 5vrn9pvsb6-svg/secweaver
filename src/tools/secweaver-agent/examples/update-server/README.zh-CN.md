...
本例只描述**已有 standalone Agent 的升级**。默认通过静态 HTTPS 服务发布无签名 manifest 和二进制，Agent 校验 HTTPS 以及文件大小、SHA-256；配置公钥和签名私钥后可启用签名清单。托管 SaaS/ES 控制面仍要求服务端资格和租约，不应复制此处的 `require_server_policy: false`。

## 前提与平台

构建机需要 Go（版本见模块 `go.mod`）、Python 3、Bash 和干净 Git 检出；离线保管的 Ed25519 发布密钥是可选项。脚本构建 Linux amd64/arm64/loong64 与 Windows amd64/arm64；本目录的 nginx 与客户端路径示例面向 Linux。Windows 必须使用对应服务安装路径并完成实际 SCM 升级、重启和健康回滚验收；交叉编译不能代替验收。

初装、授权与采集配置见 [Agent README](../../README.zh-CN.md)。本例不生成托管企业安装命令，也不部署 SaaS 服务端。

## 构建升级文件

修改 Agent 源码、打包配置或文档后，先递增 `../../VERSION` 并提交。构建读取 canonical VERSION，禁止用环境变量另取版本，也禁止在同一版本下发布不同内容。参见 [release.env.example](release.env.example)。以下命令从仓库根目录开始：
...
上面的命令生成无签名 manifest。要启用签名，先用
`go run ./cmd/update-sign -generate-key /secure/path/update-signing.key` 创建一次密钥，再在构建时
设置 `UPDATE_SIGNING_PRIVATE_KEY_FILE`，并把生成的 `.pub` 公钥配置到客户端；省略该变量即采用 HTTPS 加 SHA-256 模式。

启用签名时，私钥权限保持 `0600`，不能上传到静态服务器或仓库。客户端通过可信安装渠道获得 `.pub` 公钥。每个新签名 manifest 使用更大的 generation 和有效到期时间；不能用同一 generation 签发不同内容。当前默认二进制签名为 `ed25519-sha256`，旧客户端的过渡升级需单独验证兼容性。密钥或版本门禁失败时应修复构建输入，不绕过检查。
...
合并 [client-config.example.json](client-config.example.json) 的 `update` 块到已有配置；不要覆盖现有 modules 和采集路径。为每台设备设置稳定唯一的 `device_id`；仅在发布签名 manifest 时加入 `public_key`。确认 `manifest_url` 为 `/secweaver-agent/stable/update-manifest.json`。
...
下面是默认无签名检查；签名发布时额外加入 `-public-key BASE64_ED25519_PUBLIC_KEY`：

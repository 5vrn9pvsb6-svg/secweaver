...
按项目说明完成快速上手后，macOS、Linux 或 WSL2 使用 `make ai-showcase` 或
`.venv/bin/python src/scripts/run_ai_showcase.py --all`，按目录顺序执行全部案例，每例
最多 60 秒。Windows 用户在 WSL2 内执行这些 POSIX 命令；原生 Windows 不是受支持的
客户端运行环境。依赖安装后无需联网。
